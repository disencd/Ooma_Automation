...
def make_call_thread(pj,acc,lib,sip_url):
    global num_threads

    # Start worker thread
    thr = thread.start_new(pj.worker_thread_main, (sip_url,))
    # If argument is specified then make call to the URI
    if len(sys.argv) > 1:
        lck = lib.auto_lock()
        # Make call
        call = acc.make_call(sip_url, MyCallCallback())
        del lck
...
try:
    ...
    while (loop_cnt <= 1000):
        ...
        start_new_thread(make_call_thread, (pj,acc,lib,sys.argv[1]))
