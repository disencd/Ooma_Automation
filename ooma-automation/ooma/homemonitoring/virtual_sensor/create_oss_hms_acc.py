...
class HMS_Activation(object):
    ...
    def get_status_hms_account(self):
        ...
        time.sleep(1)
        response = HMSActions(self.json_obj, self.node).vs_oss_request_activate(self.json_server, self.url, self.cust_pk). \
        ...
        try:
            ...
            if self.mongodb_dict["activation_status"] and self.mongo_enable == "enable":
                ...
                logger.info("Getting Info of The HMS Account %s", response[0]["id"])
                _mong_obj.mongo_reset_sensor_count("SensorCount_collection",
                                                    response[0]["id"], \
                                                   self.mongodb_dict["cust_pk"])
